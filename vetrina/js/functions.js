    ...
    //Progress Bars
        inView('.progress-bar').on('enter', el => {
            var dataSource = el.getAttribute("data-value");
    ...
    //Shuffle - Filtering and masonry portfolio items
        ...
        class Demo {
            ...
            checkLightboxElems(elems) {
                ...
                if (cont) {
                    var elems = cont.querySelectorAll("[data-fslightbox]");
                    elems.forEach(el => el.removeAttribute("data-fslightbox"));
                    cont.querySelectorAll(".work-item.shuffle-item--visible a").forEach(el => el.setAttribute('data-fslightbox', 'portfolio'));
                    refreshFsLightbox();
                }
    ...
    //Ajax scripts for portfolio
        var ajaxContainer = document.getElementById("ajax-container") || false;
        if (ajaxContainer) {
            var ajaxInner = document.getElementById("ajax-inner"),
                ajaxContainer = document.getElementById("ajax-container"),
                ajaxLoader = document.getElementById("ajax-loader-animation"),
                ajaxClose = ajaxContainer.querySelector("button.close");

            document.querySelectorAll('.ajax-link').forEach(el =>
                el.addEventListener("click", function(){
                    function loadProject(){
                        let lnk = el.getAttribute("data-project");
                        ajaxInner.innerHTML = '<object id="project" data="'+lnk+'" class="fullwidth">';
                        ajaxLoader.classList.add("animating");
                        el.classList.add("active")
                        var project = document.getElementById("project");
                        project.onload = function() {
                            setTimeout(function() {
                                ajaxLoader.classList.remove("animating");
                                ajaxContainer.classList.remove("ready-to-load");
                                ajaxContainer.classList.add("content-loaded");
                            }, 400);
                        };
                        setTimeout(function() {
                    ...
                    if (ajaxContainer.classList.contains("ready-to-load")) {
                        loadProject();
                    } else if (ajaxContainer.classList.contains("content-loaded")) {
            ...
            function scrollToProjectContainer(){
                var navOffset = themeNav.getAttribute("data-offset");
                var trgt = ajaxContainer.offsetTop - navOffset;
                window.scrollTo({
                    top: trgt,
                    left: 0,
                    behavior: "smooth",
            ...
            function closeProjectContainer(){
                ajaxContainer.classList.remove("content-loaded");
                ajaxContainer.classList.add("closing");
                var elActive = document.querySelector("[data-project].active");
                elActive.classList.remove("active")
                setTimeout(function() {
                    ajaxInner.innerHTML = '';
                    ajaxContainer.classList.remove("closing");
                    ajaxContainer.classList.add("ready-to-load");
                }, 900);
            }
            ajaxClose.addEventListener("click", function(){
                closeProjectContainer()
            })
        }
    ...
    //Call content slider
        var swiperContent = new Swiper(".image-slider", {
            grabCursor: true,
            effect: "fade",
    ...
    //Vertical text slider
        const verticalTextSlider = new Swiper('.text-slider-vertical', {
            effect: 'slide',
            direction: 'vertical',
            grabCursor: false,
            touchRatio: 1,
    ...
    //*********************************************
        ...
        if (themeNav) {
            ...
            themeNav.addEventListener('click', (e) => {
                if (e.target.hasAttribute('data-toggle') && window.innerWidth <= mobileNavResolution) {
                    e.preventDefault();
            ...
            function onResizeFunction (e){
                if (this.innerWidth > mobileNavResolution) {
            ...
            var scroll = function () {
                ...
                //Add active class to parent elements when dropdown items have.
                var activeLink = themeNav.querySelectorAll(".active").forEach(function(elem){
                    if (elem.parentElement.parentElement.parentElement.classList.contains("dd-toggle")) {
                        elem.parentElement.parentElement.parentElement.querySelector("a").classList.add("active");
                    }
                });
