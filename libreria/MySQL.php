...
class MySQL
{
	private $db_host    = DBHOST; // nome del server
	private $db_user    = DBUSER;          //username del database
	private $db_pass    = DBPASS;          // password del database
	private $db_dbname  = DBNAME;          // nome del database
...
/** personalizzazione di una funziona per sfruttarla tutta con table
 * 
 */
	
	public function GetDATATABLE($showCount = true, $styleTable = null, $styleHeader = null, $styleData = null) {
	...
	*/
	...
	public function Release() {
		$this->ResetError();
		if (!$this->last_result || is_bool($this->last_result)) {
			$success = true;
		}else {
			$this->SetError();
    ...
    function scrivoErrore($message){
        $filelog_ = $filelog_ =  $_SERVER["DOCUMENT_ROOT"] . '/log/application.log';
	//die($filelog_);
	if (file_exists($filelog_))
	{
		if( filesize($filelog_) > 6000000  ){
			copy( $filelog_ , $filelog_."_" . date("YmdHis"));
			unlink($filelog_);
		}
	}	
		$a = fopen( $filelog_, "a" );        
		fwrite($a,date("d/m/Y H:i:s")." : ".$message);
