...
// cifro la password
        ...
        $this->nome_database = "admin_" . $azienda;
#data odierna
        $dataordine = date('Y-m-d');

        if ($this->Query("select amministratore.idAmministratore,amministratore.cognome, amministratore.idAzienda as Azienda,amministratore.livello,amministratore.username,amministratore.email,amministratore.pws,amministratore.idRuolo,abbonamento.idAzienda,abbonamento.scadenza, a.logo FROM admin_acecrm.abbonamento INNER JOIN  {$this->nome_database}.amministratore ON amministratore.idAzienda = abbonamento.idAzienda INNER JOIN admin_acecrm.azienda as a ON a.idAzienda=amministratore.idAzienda  WHERE scadenza > '{$dataordine}' AND username='{$azienda}' AND pws='{$pass}' AND email='{$user}'"))
            ;
        ...
        if ($this->RowCount() > 0) {
            ...
            $_SESSION['login'] = true;
            $_SESSION['tabella'] = 'admin_' . $login->username;
    ...
    public function ritrovaPassword($azienda, $email) {
        ...
        if ($this->Query("select amministratore.idAmministratore, amministratore.idAzienda as Azienda,amministratore.livello,amministratore.username,amministratore.email,amministratore.pws,amministratore.idRuolo,abbonamento.idAzienda,abbonamento.scadenza, a.logo FROM admin_acecrm.abbonamento INNER JOIN  {$nome_database}.amministratore ON amministratore.idAzienda = abbonamento.idAzienda INNER JOIN admin_acecrm.azienda as a ON a.idAzienda=amministratore.idAzienda  WHERE scadenza > '{$dataordine}' AND username='{$azienda}' AND email='{$email}'"))
            ;
